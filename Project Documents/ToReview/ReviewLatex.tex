...
Rezultatem prezentowanego projektu in¿ynierskiego jest desktopowa aplikacja bazodanowa wspomagaj¹c¹ pracê wybranej sieci hotelowej. W celu pokazania dzia³ania oprogramowania, w niniejszym rozdziale zawarta zosta³a prezentacja funkcjonalnoœci zaimplementowanych w programie. Demonstracja zosta³a przedstawiona w formie instrukcji obs³ugi poszczególnych modu³ów, obejmuj¹c opis tekstowy prezentowanych na rysunkach czynnoœci.
...
Edycja propozycji w obydwu przypadkach odbywa siê w ten sam sposób poprzez wybranie propozycji z ListView nastêpuje przypisanie identyfikatora z bazy danych do zmiennej i wyszukanie wszystkich elementów danej propozycji. Z kolei wype³niane s¹ wszystkie pola, które zosta³y zaznaczone w procesie tworzenia propozycji.
...
Aby umo¿liwiæ edycjê danych z poziomu DataGridView który jest arkuszem podzielonym na wiersze i komórki, zaprezentowanym na rysunku \ref{rys:EdytujUzytkownika} zaimplementowano metody obs³uguj¹ce zdarzenie \textit{DataGridRowEditEndingEventArgs}. W sytuacji w której edycja danej komórki zostaje ukoñczona owe zdarzenie jest przechwytywane. W tym celu wymagane jest u¿ycie zmiennej dynamic której wartoœæ sprawdzana jest podczas wywo³ania, a nie kompilacji \cite{dynamic}. Umo¿liwia to odwo³anie do nowych wartoœci zapisanych w komórkach i przekazanie ich jako argumentów do fragmentów kodu odpowiedzialnych za aktualizacjê bazy danych. Fragment kodu zawarty w programie \ref{listing:DynamicGridView} realizuje powy¿sze operacje, wykorzystywany jest w module s³u¿¹cym do edycji danych pracowników i s³owników cenowych. 

\begin{lstlisting}[caption= {Implementacja i opis fragmentu kodu aktualizuj¹cego bazê danych z poziomu DataGridView}, label={listing:DynamicGridView}]
...
Za pomoc¹ mechanizmów biblioteki MigraDoc, zaimplementowano funckje tworz¹ce tabele propozycji cenowej. Kod Ÿród³owy \ref{listing:PdfSharpTable} przedstawia implementacje mechanizmu tworz¹cego tabelê z jedn¹ kolumn¹ i wierszem. 

\begin{lstlisting}[caption= {Fragment kodu tworz¹cego prost¹ tabelê przy u¿yciu biblioteki PDFSharp \& MigraDoc}, label={listing:PdfSharpTable}]
