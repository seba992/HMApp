...
\autor{Sebastian Nalepka}{225265}{3} {~24 strony na 20.12.2015r.}
...
Dostrzegaj¹c problematykê dzia³aj¹cego obecnie rozwi¹zania dokonano rozpoznania maj¹cego na celu spisanie za³o¿eñ projektowych, które umo¿liwi¹ zniwelowanie aktualnych problemów zwi¹zanych z przeprowadzaniem operacji tworzenia nowej propozycji cenowej. W czasie rozmów ze sprzedawcami pierwszym elementem, który zosta³ przedstawiony w celu usprawnienia ich pracy by³a forma dokumentów na bazie których pracuj¹. Wykazywano inicjatywê, aby owe dokumenty dotycz¹ce cenników us³ug aktualizowane by³y bez ingerencji ka¿dego pracownika, umo¿liwiaj¹c przy tym ci¹g³¹ pracê bez obawy o posiadanie nieaktualnych danych. 

Kolejn¹ sugesti¹ ze strony pracowników by³o wycofanie koniecznoœci pracy z oprogramowaniem Microsoft Excel przez wzgl¹d na b³êdy spowodowane obowi¹zkiem dokonywania rêcznych obliczeñ oraz przypadkowym usuwaniem formu³ z pól. Problem ten polega³ na tym, i¿ tylko czêœæ pól w arkuszu, na którym przeprowadzana by³a praca nale¿a³o modyfikowaæ. W przypadku wpisania wartoœci do pola zawieraj¹cego formu³ê, owa formu³a zosta³a usuwana co skutkowa³o reakcj¹ ³añcuchow¹ polegaj¹c¹ na b³êdnym wyliczaniu wartoœci w pozosta³ych miejscach. Proces ten bardzo niekorzystnie wp³ywa³ na pracê sprzedawcy, który dodatkowo zobligowany by³ do kontroli wartoœci cenowych tworzonej propozycji oraz w sytuacji b³êdu, powtórzeniu ca³ego procesu od nowa na poprawnie dzia³aj¹cej wersji arkusza kalkulacyjnego. 

Ostatnim elementem, w sprawie którego ka¿dy ze sprzedawców dostrzeg³ problem oraz mo¿liwoœæ ulepszenia by³a rozbudowana lista produktów gastronomicznych i ich cen. Operowanie z ow¹ list¹ przez sprzedawców opiera siê na manualnym wyszukiwaniu interesuj¹cego produktu oraz sprawdzaniu jego ceny, co przez fakt du¿ej iloœci produktów jest czasoch³onne. Zasugerowano, aby ulepszenie listy polega³o na podzieleniu produktów na kategoriê oraz wyborze z ustalonej kategorii odpowiedniego produktu. Tym sposobem dostêpna lista elementów do wyboru by³aby znacznie mniejsza, co przek³ada³oby siê na znaczn¹ oszczêdnoœæ czasow¹.

Usprawnienie procesu tworzenia nowej propozycji cenowej oraz zarz¹dzania zmianami dotyczy równie¿ menad¿era sprzeda¿y. Podczas zasiêgania opinii o mo¿liwoœci usprawniania jego pracy otrzymano informacjê o chêci zaprzestania dystrybucji nowych wersji cenników oraz arkusza Microsoft Excel poprzez skrzynki elektroniczne. Proces ten okaza³ siê czasoch³onny poprzez czêste aktualizacje danych i obowi¹zek ich przesy³ania ca³ej kadrze pracowników, w której dodatkowo nastêpuj¹ liczne rotacje osobowe. Przy tym punkcie dostrze¿ono jednoczeœnie mo¿liwoœæ wprowadzenia centralnego zarz¹dzania kontami pracowników, które umo¿liwi³yby w prosty sposób wdro¿enie nowych osób oraz usuniêcie starych.

Drugi zas³yszany element usprawniaj¹cy pracê menad¿era to mo¿liwoœæ przegl¹dania utworzonych przez pracowników propozycji cenowych. W chwili obecnej opcja ta jest niemo¿liwa do wykonania pomijaj¹c fakt dostarczenia menad¿erowi propozycji drog¹ elektroniczn¹ przez pracownika. Rozwi¹zanie to implikuje jednak koniecznoœæ dodatkowej pracy sprzedawcy, a w przypadku zmodyfikowania wys³anej propozycji cenowej obowi¹zku poinformowania o tym fakcie menad¿era oraz wys³ania zaktualizowanej wersji w celu weryfikacji.

Wa¿nym elementem wyci¹gniêtym z rozpoznania zosta³ widoczny na rysunku \ref{rys:SzablonPropozycji} szablon PDF w postaci arkusza kalkulacyjnego, który opracowany zosta³ na podstawie burzy mózgów oraz aktualnej wersji dokumentu, na bazie której tworzona jest propozycja cenowa. Szablon ten w znacznym stopniu u³atwia rozlokowanie wszelkich elementów oraz ukazuje iteracyjny proces tworzenia propozycji, który nale¿y zaimplementowaæ w tworzonej aplikacji. Przedstawia on po¿¹dane przez pracowników u³o¿enie elementów propozycji oraz ukazuje wszelkie zale¿noœci, które wystêpuj¹ w czasie jej tworzenia.
...
Dokonuj¹c podsumowania przedstawionych problemów oraz jej analizy przez zespó³ utworzono ogóln¹ listê za³o¿eñ, których realizacja konieczna jest w celu uprawnienia procesu tworzenia propozycji cenowej ze strony zarówno sprzedawcy, jak i menad¿era. Przedstawiona lista zawiera tak¿e elementy, które na drodze dedukcji uznane zosta³y przez zespó³ jako warte wprowadzenia.

\begin{itemize}

\item \textsl{Wprowadzenie dwóch typów kont u¿ytkowników przez fakt na ró¿ny zakres obowi¹zków s³u¿bowych,}

\item \textsl{Aktualizacja wszelkich danych bez koniecznoœci ingerencji osobistej sprzedawcy (aktualizacja ze strony menad¿era automatycznie powinna byæ dostêpna na kontach sprzedawców),}

\item \textsl{Zaprzestanie aktualizacji danych za pomoc¹ skrzynek elektronicznych (aktualizacja cenników z poziomu konta administratora),}
...
\item \textsl{Zautomatyzowanie przeprowadzanych obliczeñ w miejscach, w których jest to mo¿liwe (odci¹¿enie sprzedawcy),}

\item \textsl{Podzia³ produktów gastronomicznych na kategorie i mo¿liwoœæ wyboru szukanego elementu z krótkiej listy,}

\item \textsl{Wprowadzenie centralnego zarz¹dzania pracownikami z poziomu dedykowanego dla ka¿dej osoby konta,}

\item \textsl{Umo¿liwienie dodawania, edycji, usuwania kont z poziomu konta administratorskiego,}

\item \textsl{Mo¿liwoœæ przegl¹dania tworzonych przez sprzedawców propozycji cenowych z poziomu konta administratorskiego}

\item \textsl{Walidacja wprowadzanych przez u¿ytkownika danych oraz informowanie go o potencjalnych b³êdach,}

\item \textsl{Funkcja tworzenia dokumentu PDF zapisanej propozycji cenowej,}
...
W rozdziale \ref{dzial:geneza} przedstawiono problemy sieci hotelowej, wystêpuj¹ce podczas przeprowadzania transakcji z klientem. S¹ one podstawowymi czynnikami determinuj¹cymi docelow¹ formê aplikacji, dobór narzêdzi i technologii oraz wymagania które nale¿y spe³niæ w celu wdro¿enia proponowanego rozwi¹zania. Pierwszym istotnym elementem infrastruktury IT w firmach jest to, jaki typ systemów operacyjnych jest u¿ywany. Jak pokazuj¹ rankingi, jednymi z najczêœciej u¿ywanych systemów operacyjnych s¹ systemy rodziny Windows \cite{Systemy} takie jak Windows 7, 8. W kontekœcie doboru technologii oznacza to, i¿ warto zastosowaæ rozwi¹zania firmy Microsoft które s¹ w pe³ni zintegrowane z systemami Windows, ze wzglêdu na ich wysok¹ wydajnoœæ oraz popularnoœæ. Fundamentalnym elementem aplikacji jest baza danych w której przechowywane s¹ detale transakcji, szczegó³owe listy oferowanych us³ug czy dane osobowe pracowników. Firma Microsoft oferuje produkt SQL Server, który posiada darmowe dystrybucje i jest oprogramowaniem bazodanowym dedykowanym dla jêzyków platformy programistycznej .NET Framework. SQL Server w wersji darmowej mo¿e zostaæ u¿yty do w projektach czysto hobbystycznych jak i w pe³ni komercyjnych co jest jego du¿ym atutem. W rankingu portalu \textsl{db-engines.com} SQL Server zajmuje 3 pozycjê, nieprzerwanie od 2014 roku poœród ponad 200 sklasyfikowanych silników baz danych \cite{SQLRanking}. Wystarczaj¹cym kryterium które musi zostaæ spe³nione aby firma mog³a korzystaæ z aplikacji bazuj¹cej na tym silniku, jest przygotowanie jednego serwera centralnego z którym za poœrednictwem aplikacji ³¹cz¹ siê pracownicy. Opisane rozwi¹zanie eliminuje jeden z problemów, którym jest synchronizacja danych pomiêdzy pracownikami, takich jak aktualne wersje propozycji cenowych oraz listy oferowanych us³ug. 
...
Jednym z ostatnich aspektów wytwarzania oprogramowania jest wybranie, jakiego typu bêdzie to aplikacja. Za³o¿eniem niniejszego projektu in¿ynierskiego jest implementacja aplikacji desktopowej. Zalety tradycyjnych aplikacji desktopowych, które spowodowa³y wybór tego typu s¹ nastêpuj¹ce:
...
W kontekœcie wdro¿enia, aplikacja powinna zostaæ zainstalowana na stacjach roboczych pracowników co jest drugim warunkiem koniecznym do wprowadzenia nowej infrastruktury IT dla pracowników dzia³u sprzeda¿y. Je¿eli w przysz³oœci aplikacja by³aby rozwijana, za pomoc¹ narzêdzi Continuous Integration kolejne wersje oprogramowania mog¹ zostaæ szybko dostarczone sieci hotelowej w postaci paczki zip, co zosta³o szerzej opisane w rozdziale \textsl{Automatyzacja wydawania kolejnych wersji aplikacji}. 
...
Do sprawdzenia dostêpnych rozwi¹zañ zosta³y wytypowane trzy programy, które spe³niaj¹ wymagania sieci hotelowej s¹ to: ProfitCRM\cite{ProfitCRM} i Asystent CRM\cite{AsystentCRM} Gestor GT \cite{Gestor}. Zosta³y one wytypowane z powodu mo¿liwoœci pod³¹czenia do istniej¹cej ju¿ infrastruktury w firmie opartej na badzie MS-SQL. Eliminuje to koniecznoœæ ingerencji sprzedawcy w proces aktualizowania danych. Wa¿nym czynnikiem wyboru tych programów jest mo¿liwoœæ generowania PDF zapisanych propozycji. Czynnikiem wp³ywaj¹cym jest równie¿ przejrzystoœæ interfejsu u¿ytkownika. Ma to istotne znaczenie, poniewa¿ nie ka¿dy u¿ytkownik mia³ kontakt z tego typu systemem wspomagaj¹cym sprzeda¿. Wykorzystanie aplikacji  desktopowej gwarantuje nam równie¿ wiêksz¹ kontrolê nad dostêpem do danych. Element ten jest istotny z punktu bezpieczeñstwa. Przydatn¹ funkcj¹ jest integracja  z skrzynk¹ pocztow¹, której wy³¹cznym zadaniem jest kontakt z klientem. 
...
\item \textsl{Tworzenie szablonów dokumentów PDF} 

\tab W programie ProfitCRM  istnieje równie¿ mo¿liwoœæ zdefiniowania w³asnego szablonu. Proces tworzenia nowego szablonu jest procesem skomplikowanym, ale naj³atwiejszym z poœród reszty. Najlepszym rozwi¹zanie jest skopiowanie istniej¹cych w systemie szablonów, poniewa¿ ich edycja jest zablokowana.  Nastêpnie edytowanie skopiowanego. Nie powiod³o siê równie¿ wygenerowanie PDF o wygl¹dzie zbli¿onym do wygl¹du przedstawionego przez sieæ hotelow¹. Niestety w programie Asystent CRM edycja wzorów dokumentów do druku jest bardzo trudna w realizacji dla zwyk³ego sprzedawcy. Jest to spowodowane sposobem edycji. Bez znajomoœci
programowania nie ma mo¿liwoœci stworzenia nowej wersji dokumentu, co utrudnia zmianê wygl¹du pod dana firmê. W ostatnim przetestowanym programie czyli Gestor GT nie ma ¿adnej mo¿liwoœci edycji szablonów dokumentów.
...
\item \textsl{Integracja z skrzynk¹ pocztow¹}
 
\tab
	Przydatn¹ funkcj¹ jest integracja z elektroniczn¹ skrzynk¹ pocztow¹. W programie ProfitCRM wykorzystuje siê w tym celu zewnêtrzne programy takie jak  MS Outlook i Mozilla Thunderbird. Umo¿liwia on równie¿ gromadzenie ca³ej korespondencji z kontrahentem w jednym miejscu nawet jeœli komputer, z którego zosta³a wys³ana wiadomoœæ jest wy³¹czony. W Asystent CRM istnieje zintegrowany klient pocztowy, który umo¿liwia pod³¹czenie wielu kont z ró¿nych serwisów pocztowych bez koniecznoœci instalowania klienta poczty elektronicznej. Gestor GT ma zbli¿one rozwi¹zanie w stosunku do Asystent CRM. Miedzy innymi tworzenie szablonów maili. 

\end{itemize}

Podsumowuj¹c powy¿sze programy mo¿na zauwa¿yæ, ¿e proces dodawania produktów jest bardzo zbli¿ony, odbywa siê poprzez nowo otwarte okno. Proces wystawiania faktur, ofert jest taki sam odbywa siê to przez klikniecie w guzik oferta/faktura. Ka¿dy z nich udostêpnia równie¿ bardo du¿¹ iloœæ zdefiniowanych szablonów wystawiania dokumentów. Dwa z nich ProfitCRM, Asystent CRM w wersji testowej mia³y równie¿ mo¿liwoœæ tworzenia nowych szablonów dla dokumentów. Najlepszym rozwi¹zanie w tworzeniu szablonów dokumentów ma program ProfitCRM. Przenosz¹c predefiniowanych elementów np. pola tekstowa. Umo¿liwia równie¿ tworzenie  wielu wariantów jednej oferty, u³atwia to proces dostosowania oferty do konkretnego klienta. Warianty te mog¹ siê ró¿niæ np. walut¹. Istotn¹ z punktu widzenia relacji z klientem jest sprawdzanie zadowolenia klienta z obs³ugi sieci hotelowej umo¿liwia to wy³¹cznie ProfitCRM poprzez system ankiet. Niestety brak podzia³u na grupy towarów w programie Gestor GT utrudnia zarz¹dzanie nimi i tworzenie ofert. Jedyna mo¿liwoœci¹ tworzenia propozycji cenowej jest edycja szablonów dokumentów z ofert¹ w sposób zbli¿ony do przedstawionego w analizie dokumentu.

\section{Pomys³ na rozwi¹zanie problemu}
Analizuj¹c powy¿sze rozwi¹zania i aspekty zwi¹zane z wymaganiami stawianymi przed aplikacj¹, wynik³a koniecznoœæ zrealizowania dedykowanej aplikacji, której zadaniem jest tworzenie propozycji cenowej zgodnie z wymaganiami sieci hotelowej. Najbardziej istotnym element programu jest mo¿liwoœæ generowania PDF propozycji cenowej bez zbêdnego wysi³ku z strony sprzedawcy. Niestety program tego nie realizowa³ w prosty sposób. Jest to zwi¹zane z problem prawnym s³owa ,,oferta" poniewa¿ zgodnie z treœci¹ art. 71 Kodeksu Cywilnego  propozycja cenowa nie stanowi oferty handlowej, a jedynie pe³ni funkcjê informacyjn¹. Zosta³ wykorzystany sposób podzia³u u¿ytkowników na dwie grupy: administrator i u¿ytkownik co odpowiada roli menad¿era i sprzedawcy. Z stanowiskiem  menad¿era wi¹¿e siê mo¿liwoœæ podgl¹dania propozycji wystawianych przez sprzedawców. Zosta³o to zrealizowane w oknie g³ównym programu w postaci tabeli. By nie utrudniaæ nawigacji w oknie menad¿era niektóre z elementów zosta³y przeniesione do osobnych okien.  Kolejnym wymaganiem przedstawionym przez sieæ hoteli zwi¹zanym z aktualizacj¹ wszelkich danych wykorzystanych do tworzenia propozycji, zostanie stworzone  wyspecjalizowane okno z wyznaczonymi elementami do zmiany cen produktów. Ka¿dy u¿ytkownik równie¿ administrator aplikacji ma swój dedykowany element sk³adowy okna g³ównego, w którym jest mo¿liwoœæ dodawania, edycji i zapisywania propozycji cenowej.Do edycji wykorzystany zostanie element tworz¹cy now¹ propozycje, by nie duplikowaæ elementów. Administrator poprzez osobne okna ma mo¿liwoœæ dodawania, usuwania i edycje kont sprzedawców.   

\section{Zastosowane rozwi¹zania technologiczne}

Niniejszy rozdzia³ zawiera opis technologii wykorzystanych do implementacji prezentowanej aplikacji. Zakres opisu obejmuje wszelkie komponenty techniczne, które znalaz³y zastosowanie w projekcie:
...
\item \textsl{Narzêdzia wspieraj¹ce pracê zespo³u programistycznego,}
\item \textsl{Biblioteki umo¿liwiaj¹ce wdro¿enie sprawdzonych ju¿ i poprawnie dzia³aj¹cych rozwi¹zañ skomplikowanych operacji.}
\end{itemize}
...
\begin{itemize}
	...
	\tab 
	Jest to ca³kowicie darmowe narzêdzie do u¿ytku domowa i komercyjnego. Ograniczone jest do konkretnych rozwi¹zañ na przyk³ad aplikacji komputerowych, stron internetowych lub na platformy mobilne. 
...
\begin{figure}[ht]
    ...
    \caption{Czêœci s³ownikowej bazy, ród³o: Opracowanie w³asne.}
...
% 
\bibitem {SystemERP}
Kisielnicki J., Pañkowska M., Sroka H.: \textsl{Zintegrowane systemy informatyczne. Dobre praktyki wdro¿eñ systemów klasy ERP}, Wydawnictwo Naukowe PWN, Warszaw, 2012
