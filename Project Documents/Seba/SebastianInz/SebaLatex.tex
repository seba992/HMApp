...
\tab Oparte o architekturê typu klient-serwer. Starsze wersje plików przechowywane s¹ tylko na serwerze.

\item \textsl{Rozproszone (np. Git)}

\tab Opartê o architekturê P2P. W odró¿nieniu od systemów scentralizowanych, archiwalne wersje plików przechowywane s¹ zarówno na serwerze jak i na komputerach programistów, co zabezpiecza przed utrat¹ danych.
...
\begin{figure}[ht]
    ...
    \caption{Atlassian SourceTree, ród³o: Opracowanie w³asne.}
\end{figure}
GitHub.com jest darmowym serwisem internetowym który pozwala na przechowywanie projektów programistycznych, wspiera system kontroli wersji Git. Pierwsza wersja zosta³a zamieszczona na serwerze, w kwietniu 2008 roku \cite{github}. GitHub umo¿liwia m. in. tworzenie i zarz¹dzanie repozytoriami (np. nadawanie uprawnieñ dostêpu wspó³pracownikom), podgl¹d plików danego projektu. Ciekaw¹ funkcjonalnosci¹ serwisu jest równie¿ opracowywanie statystyk u¿ytkowników, i prezentowanie ich wk³adu w konkretny projekt w formie wykresów.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\textwidth]{github.jpg}
    \caption{Statystyka wk³adu z dnia 13 grudnia 2015 r., ród³o: Opracowanie w³asne.}
\end{figure}
...
Jenkins Continuous Integration Server jest darmowym, wieloplatformowym narzêdzien s³u¿acym do ci¹g³ej integracji projektów programistycznych. Zosta³ napisany w jêzyku Java, wywodzi siê od narzêdzia Hudson. Zosta³ wydany po raz pierwszy w 2011 r. \cite{jenkins}. Posiada szereg wtyczek zapewniaj¹cych funkcjonalnoœci takie jak statyczna analiza kodu, automatyczne tworzenie dokumentacji kodu Ÿród³owego, powiadomienia e-mail. Pozwala na budowanie oraz testowanie projektów w popularnych jêzykach programowania, m. in. w jezyku C\# czy Java. Jenkins jest aplikacj¹ internetow¹, domyœlnie dzia³aj¹ca na porcie 8080. Zadania do wykonania definiowane s¹ w postaci planów (ang. job) które mog¹ byæ uruchamiane cyklicznie, zgodnie z okreœlonym harmonogramem lub manualnie \cite{jenkins2}.
...
\subsubsection{Programy i pluginy u¿yte w Jenkins}
\subsubsection{Schemat budowania programu}

Kolejne wersje niniejszego projektu s¹ wydawane zgodnie z zasadami Continuous Delivery. Plan \textit{Build Project} pobiera kod Ÿród³owy z repozytorium zapisanym na GitHubie. CA£A SEKCJA DO DOKONCZENIA.
...
Zostaje wyœwietlona lista propozycji cenowych. Po klikniêciu przycisku \textit{Zapisz do PDF}, pojawia siê dialog wyboru plików. U¿ytkownik wpisuje nazwê pliku oraz lokalizacjê zapisu. 
...
W przeciwnyk wypadku, u¿ytkownik otrzyma komunikat o b³êdzie.
...
Aby umo¿liwiœc edycjê danych z poziomu DataGridView, zaimplementowano funckjê obs³uguj¹c¹ zdarzenie DataGridRowEditEndingEventArgs. Kiedy edycja danje komórki zostaje zakoñczona, wywo³ywane jest wnêtrze funkcji której zadaniem jest dynamiczne przechwycenie nowej wartoœci i aktualizacja bazy danych. U¿ycie zmiennej dynamic, podyktowane jest tym ¿e jest ona sprawdzana podczas wywo³ania, a nie kompilacji \cite{dynamic} co umo¿liwia odwo³anie do nowych wartoœci zapisanych w komórkach. Kod funkcji jest jest nastêpuj¹cy: 
