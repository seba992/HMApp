...
Odnoszπc siÍ do planowania architektury systemu bardzo pomocne okazujπ siÍ dostÍpne wzorce architektoniczne. Wzorce te sπ to sprawdzone oraz zaakceptowane sposoby rozwiπzania okreúlonego problemu z dziedziny architektury oprogramowania, okreúlajπce ogÛlnπ strukturÍ systemu informatycznego, zasady komunikacji pomiÍdzy komponentami oraz elementy wchodzπce w jego sk≥ad wraz z opisem ich funkcji. WybÛr odpowiedniego wzorca w duøej mierze zaleøny jest od wykorzystanej w projekcie technologii. W przypadku WPF powszechnie uznanym rozwiπzaniem jest MVVM (Model View ViewModel), ktÛry jest zmodyfikowanπ wersjπ wzorca MVC, zawierajπcπ specjalizacjÍ modelu prezentacji.
...
\subsubsection{Dostosowanie Entity Framework do wymagaÒ zwiπzanych z za≥oøeniami projektowymi oraz wzorcem MVVM}

- ulokowanie, budowa, screen, zmiana na obserwowalne kolekcje
...
W celu wykonania polecenia SELECT zwracajπcego rekordy w bazie danych naleøy za pomocπ przedstawionego schematu wybraÊ nazwÍ tabeli z obiektu wygenerowanego przez Entity Framework a nastÍpnie uøyÊ polecenia \textsl{where} definiujπcego wyszukiwanπ zaleønoúÊ. Finalnie poleceniem \textsl{select} ustaliÊ moøna wczeúniej zdefiniowanπ zwracanπ zmiennπ, w ktÛrej przechowywane sπ wyszukane elementy. Tym sposobem dane przechowywane w bazie danych poddane zosta≥y translacji na poziom obiektÛw, z ktÛrych moøliwe jest korzystania z poziomu C\# w sposÛb standardowy dla obiektÛw.
...
Edycja danych znajdujπcych siÍ w bazie danych z poziomu projektu programistycznego realizowana jest poprzez modyfikacjÍ opisanego polecenia \textsl{SELECT}. Modyfikacja ta polega na przekszta≥ceniu otrzymanego obiektu na typ klasy modelu Entity, ktÛra reprezentuje 

Po tak przeprowadzonej operacji otrzymujemy obiekt kolekcji \textsl{IQueryable}, ktÛry za pomocπ metody \textsl{ToList} moøna bezproblemowo przekszta≥ciÊ na ListÍ o typie zadanym nazwπ wyszukanej tabeli Entity modelu, na ktÛrej 
TO DO

- opis linq to entities (select)
- insert
- update
...
SCREEN B£ DNEGO WPISANIA DANYCH
...
% Windows 8. Programowanie aplikacji z wykorzystaniem C# i XAML <- o MVVM
